...
class Adjustment(models.Model):
    ...
    def action_validate(self):
        """
        Override to validate without check tracking products if company allow (Track Serial in Sales Only)
        """
        if not self.exists():
            return
        self.ensure_one()
        ...
        if inventory_lines and not lines and not self.company_id.track_serial_sales_only:
